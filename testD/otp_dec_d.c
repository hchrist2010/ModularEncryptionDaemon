		...
		if(establishedConnectionFD >= 0){
			...
			if(pid == 0){
				...
				if(strcmp(buffer, "otp_dec") == 0){
					...
					error("Attempted to connect to wrong daemon\n");
					exit(2);
				}
				...
				//reveive the file in packets of 255, continue to reveive until the full file has been received
				//store the file in the enc string and then send back a 0 indicating the string has been received
				int counter = 0;
				char *enc;
				enc = (char*)malloc(fileLength + 256);
...
//				printf("here\n");
				...
				//receive the key in packets of 255, continue to receive until the full key has been received
				//store the key in teh key string then send back 0 indicating the key has been fully received
				counter = 0;
				key = (char*)malloc(keyLength + 256);
				while(counter < keyLength - 1){
					memset(buffer, '\0', 256);
					charsRead = recv(establishedConnectionFD, buffer, 255, 0); // Read the client's message from the socket
					counter += charsRead;
					strncat(key, buffer, 255);
				}
				...
				d();
				//dycrypt the file by subracting 65, this will get a number between 0 and 26
				//then subtract the key value at the index from it,
				//modulo 27, then, because the number could have been negative, add 27 then modulo 27 again
				//finally, add 65 and store the reulting ascii value at the index in enc
				//lastly, if index is a '[', change it to ' '
				for(i = 0; i < fileLength - 1; i++){
					enc[i] -= 65;
