		...
		if(establishedConnectionFD >= 0){
			...
			if(pid == 0){
				...
				while(counter < fileLength - 1){
					...
					charsRead = recv(establishedConnectionFD, buffer, (packet) - 1, 0); // Read the client's message from the socket
				//	printf("%d\n", charsRead);
					sprintf(tempch, "%d", charsRead);
					charsWritten = send(establishedConnectionFD, tempch, strlen(tempch), 0);

					memset(tempch, '\0', packet);
					charsRead = recv(establishedConnectionFD,tempch, (packet), 0);
					temp1 = atoi(tempch);

					i = 0;
					if(temp1 == 0){
						while(buffer[i] != '\0'){
							enc[i + counter] = buffer[i];
							i++;
							//printf("%c\n", enc[i + counter]);
						}
						counter += i;
				...
				charsRead = send(establishedConnectionFD, "asdf", strlen("asdf"), 0);
				charsRead = recv(establishedConnectionFD, buffer, (packet - 1), 0); // Read the client's message from the socket
				d();
