...
#define packet 32	//This controls the size of the packet the files are received in
...
int main(int argc, char *argv[])
{
	...
	//the file will be received in packets of (packet - 1) bytes
	memset(enc, '\0', sizeof(enc));
	int counter = 0;
	while(counter < fileLength - 1){
		memset(buffer, '\0', packet);
		charsRead = recv(socketFD, buffer, (packet - 1), 0); // Read the client's message from the socket
		counter += charsRead;
