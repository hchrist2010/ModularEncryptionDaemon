...
int main(int argc, char *argv[])
{
	...
	charsWritten = send(socketFD, "otp_enc", strlen("otp_enc"), 0);
	...
	//Variable declarations for sending the key and file to be encoded
	FILE *fp;
	FILE *keyFile;
	char ch;
	char *enc;
...
//		printf("%s\n", buffer);
		if(temp = strlen(buffer) - 1){
			charsWritten = send(socketFD, "0", strlen("0"), 0);
			count += packet;
		}
		else{
			charsWritten = send(socketFD, "1", strlen("1"), 0);
		}
	}
	...
	if(strcmp(buffer, "1") == 0){
		fprintf(stderr, "File longer than key\n");
		exit(1);
	}
